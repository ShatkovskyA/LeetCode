...
public class _657_Robot_Return_to_Origin_ {
  ...
  static class Solution657 {
    ...
    // метод лпределяет, возвращается ли робот в исходное положение после выполнения последовательности ходов
    public boolean judgeCircle(String moves) {
      // вводим и инициализируем две переменные - это начало координат
      // смещение горизонтальной оси
      int x = 0;
      // мещение вертикальной оси
      int y = 0;

      // далее вводим цикл, и пробегаемся по этому циклу, имитируя, с помощью условий - движения
      // т. е. обрабатываем каждое перемещение строки перемещений
      // for (int i = 0; x < moves.length(); ++i) {

      // второй вариант
      for (char move : moves.toCharArray()) {

        // сперва получаем текущий ход - метод charAt(i), получающий i-й символ,
        // вводом если классический for
        // char move = moves.charAt(i);
        // перебираем буквы которые могут быть в строке "moves", согласно условиям,
        // и расчитываем смещение
        if (move == 'R') {
          // это вправо
          // x++;
          x += 1;
        ...
        // примечание: Нет регистра "else",
        // т. к., что все символы в "moves" допустимы (только "R", "L", "U", "D").
      }

      // проверка, вернулся ли робот в исходное положение (0,0)? && - логическое И
      return (x ==0 && y == 0);
    }
  }
